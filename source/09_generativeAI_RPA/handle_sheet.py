import os
import datetime 
import shutil # 파일 및 디렉토리 작업을 쉽게 할 수 있도록 도와주는 라이브러리
...
def update_now_list(wb, df_shopping):
    # 6. 'now_list' 시트의 모든 내용을 지우고, df_shopping 내용을 'A1'셀에 업데이트
    now_sheet = wb.sheets['now_list']
    now_sheet.clear()
...
def update_now_report(wb, analysis, summary):
    ...
    if 'prev_report' in sheet_names:
        ...
        print('prev_report 시트가 존재하지 않아 삭제 못함')
    ...
    if 'now_report' in sheet_names:
        ...
        print('now_report 시트가 없어 작업을 중단합니다.')
    ...
    # now_sheet.range('A3').api.HorizontalAlignment = -4152 (xw.constants.HAlign.xlHAlignRight 프린트하면 나오는 숫자)
    # analysis는 A5셀에 입력
