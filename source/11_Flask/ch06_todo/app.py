...
def create():
    ...
    todo_data[todo.id] = todo.model_dump() # todo 객체를 dict 형태로 변환하여 todo_data에 추가 
...
@app.route('/update/<int:id>/<string:content>/<string:is_done>', methods=['PUT']) # 수정할 수 있는 페이지 가기
...
@app.route('/delete/<int:id>', methods=['DELETE']) # 수정할 수 있는 페이지 가기
def delete(id):
    todo = todo_data.get(id)
    if todo:
        del todo_data[id]
