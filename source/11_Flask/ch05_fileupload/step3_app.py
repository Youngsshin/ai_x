...
from werkzeug.utils import secure_filename # 업로드한 파일명에 특수문자를 제거해서 저장
...
class FileForm(FlaskForm):
    files = FileField('file', validators=[FileRequired()]) # 업로드한 파일 객체 
...
def index():
    ...
    if form.validate_on_submit(): # 폼 데이터가 유효한지(POST 요청이 유효하게 들어왔는지) 체크
        ...
        infos = [] # 파일 정보(파일명, 생성시점, 최종수정시점, 사이즈) 리스트
