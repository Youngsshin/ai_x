...
def info(filename): 
    ctime = os.path.getctime(UPLOAD_FOLDER + filename) # 파일 생성 시간
    mtime = os.path.getmtime(UPLOAD_FOLDER + filename) # 파일 최종 수정 시간 
    atime = os.path.getatime(UPLOAD_FOLDER + filename) # 파일 최종 접근 시간
    size = os.path.getsize(UPLOAD_FOLDER + filename) # 파일 크기(byte 단위)
    if size >= 1024 * 1024:
        size = size / (1024 * 1024)
