...
class Post(models.Model): # 테이블명 : blog_post
    ...
    title = models.CharField(verbose_name="제목",
                            max_length=100) # VARCHAR 타입은 최대 길이 반드시 지정해야함
    content = models.TextField(verbose_name="본문") # 최대 길이 제한 없음 CLOB, TEXT 타입
    create_at = models.DateField(auto_now_add=True) # 자동으로 지금 시간 저장
    ...
    lnglat = models.CharField(verbose_name="경도, 위도",
                            ...
                            blank=True,
                            null=True,
                            help_text="경도, 위도 포맷으로 저장", # 38.5, 125.8 / 38,125
...
class Comment(models.Model): # 테이블명 : blog_comment (Post의 댓글 내용-post:comment=1:N)
    ...
    update_at = models.DateTimeField(auto_now=True)
    def __str__(self):
        return "{}번 글의 댓글 - {}(by {})".format(self.post.pk, self.message, self.author)
    class Meta:
