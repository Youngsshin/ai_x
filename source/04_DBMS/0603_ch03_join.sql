...
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, E.DEPTNO, DNAME, LOC  -- (3)
    FROM EMP E, DEPT D   -- (1)
    ...
    -- ex3) 근무지(LOC)가 'CHICAGO'인 직원의 이름, 직책, 급여, 부서번호 출력
    SELECT ENAME, JOB, SAL, D.DEPTNO
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND LOC='CHICAGO';
    -- ex4) 82년도에 입사한 10, 20번 부서직원의 이름, 급여, 근무지 출력(급여순)
    ...
    --      이름, 급여, 상여, 연봉, 부서명, 근무지(연봉이 큰 순) 출력
    SELECT ENAME, SAL, COMM, SAL*12+NVL(COMM, 0) 연봉, DNAME, LOC
        FROM EMP E, DEPT D
...
SELECT *
    ...
    -- ex1) 모든 사원의 사번, 이름, 직책, 상사사번, 급여, 급여등급(1등급, 2등급...)
    SELECT EMPNO, ENAME, JOB, MGR, SAL, GRADE||'등급' GRADE
        FROM EMP, SALGRADE
        WHERE SAL BETWEEN LOSAL AND HISAL;
    -- ex2) 모든 사원의 사번, 이름, 직책, 상사사번, 급여, 급여등급(1등급, 2등급...) 부서명
...
--	탄탄다지기 연습문제
    ...
    SELECT ENAME, SAL, HIREDATE, GRADE
        FROM EMP, SALGRADE
        WHERE SAL BETWEEN LOSAL AND HISAL;
    --	이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력.
    --  부서가 같으면 연봉순. 연봉=(sal+comm)*12 comm이 null이면 0
    SELECT ENAME, SAL, GRADE, SAL*12+NVL(COMM,0) 연봉, DNAME, COMM
...
SELECT W.ENAME, W.SAL, D.DEPTNO, DNAME, LOC, M.ENAME
