-- DCL(계정생성, 계정부여, 권한박탈, 계정삭제, 트랜젝션 명령어)
...
show databases; -- 데이터 베이스 list
create database devdb; -- 데이터 베이스 생성
use devdb; -- 위에서 생성한 devdb(특정데이터베이스)로 들어감 
select database(); -- 현재 들어와 있는 데이터 베이스를 보여줌
drop table emp;
...
create table major(
	mcode int primary key auto_increment, -- int로 해야함
    mname varchar(30),
    moffice varchar(30)
)default charset=utf8;
...
insert into major (mname, moffice) values ('정보통신', 'a104호');
select * from major;
...
select * 
    ...
    where s.mcode = m.mcode; -- equi, non-equ, self join은 사용법 동일
select * 
...
insert into student values (103, '이길동', 9); -- 에러 : fk
select * from student;
select sno, sname, m.mcode, mname, moffice
	from major m, student s
	where m.mcode = s.mcode; -- 3번 학과는 출력되지 않음(이유: student에 없어서)
select sno, sname, m.mcode, mname, moffice
...
-- 7. 사번, 이름, 상사이름
select p.pno, p.pname, l.pname mng
	from personal p, personal l
    where p.manager = l.pno;

-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) 
select p.pno, p.pname, ifnull(l.pname, '★CEO★') mng
	from personal p left outer join personal l
    on p.manager = l.pno;
...
select w.pno, w.pname, m.pname, dname
    ...
    where w.manager=m.pno && d.dno=w.dno;
...
select * from personal;
select pname, pay, ifnull(bonus, 0) bonus
	from personal;
select pname, pay, if(bonus is null, 0, bonus) bonus
	from personal;
select pname, pay, if(pay>=3000, '부자', '평범') pay2
	from personal;
